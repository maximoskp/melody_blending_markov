...
class EvoSession:
    ...
    def __init__(self, file1_name, file2_name, target_features, target_markov, nPop=300, nGen=200, print_gens=True):
        ...
        self.nPop = nPop
        self.nGen = nGen
        self.operator_probabilities = np.array([ 0.0, 1.0 ]) # PUT BACK TO 0.5
        # TARGETs
    ...
    def bar_rhythm_exchange(self, p1, p2):
        ...
        print('measure_index_1: ', measure_index_1)
        measure_1.show('t')
        print('measure_index_2: ', measure_index_2)
        measure_2.show('t')
    ...
    def evolution_round(self):
        ...
        while len(self.nextPop) < self.nPop:
            ...
            # TO BRING BACK
            # TO DELETE
            p1 = self.prevPop[0].stream
            p2 = self.prevPop[19].stream
            # TO DELETE
            ...
            if idx_op[0] == 0:
                ...
                print('single_note_exchange')
                # TO DELETE
                fef.write_stream_to_midi(p1, appendToPath='tmp_trash/', fileName='snx_p1.mid')
                fef.write_stream_to_midi(p2, appendToPath='tmp_trash/', fileName='snx_p2.mid')
                fef.write_stream_to_midi(c1, appendToPath='tmp_trash/', fileName='snx_c1.mid')
                fef.write_stream_to_midi(c2, appendToPath='tmp_trash/', fileName='snx_c2.mid')
                # TO DELETE
                time.sleep(10.0)
            elif idx_op[0] == 1:
                c1, c2 = self.bar_rhythm_exchange(p1, p2)
                print('bar_rhythm_exchange')
                # TO DELETE
                fef.write_stream_to_midi(p1, appendToPath='tmp_trash/', fileName='brx_p1.mid')
                fef.write_stream_to_midi(p2, appendToPath='tmp_trash/', fileName='brx_p2.mid')
                fef.write_stream_to_midi(c1, appendToPath='tmp_trash/', fileName='brx_c1.mid')
                fef.write_stream_to_midi(c2, appendToPath='tmp_trash/', fileName='brx_c2.mid')
                # TO DELETE
                time.sleep(10.0)
            else:
